...
def get_chimeric_aff(vcf_file, samples):
    ...
    df.columns = ['position', 'x', 'n', 'folded']
    #Remove duplicates, keeping the higher value (ie in case the beneficial has overlayed a previous mutation)
    df = df.groupby('position', group_keys=False).apply(lambda x: x.loc[x['x'].idxmax()]).reset_index(drop=True)
    return(df)


#Get site counts from WF vcf file
def get_WF_aff(vcf_file, samples):
    vcf_reader = vcf.Reader(filename=vcf_file)
    d_af = {}
    for i,record in enumerate(vcf_reader):
        #create list of genotypes
...
if(sample_type=='chimeric'):
    aff = get_chimeric_aff(vcf_file, samples)
else:
    aff = get_WF_aff(vcf_file, samples)

aff.to_csv(out_file + '.aff', sep='\t', header=True, index=False)
aff[['position']].to_csv(out_file + '.grid', sep='\t', header=False, index=False)
